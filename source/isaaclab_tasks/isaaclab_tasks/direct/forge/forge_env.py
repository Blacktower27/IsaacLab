...
class ForgeEnv(FactoryEnv):
    ...
    def __init__(self, cfg: ForgeEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print(f"[ForgeEnv] robot body_names: {self._robot.body_names}")
        self.force_sensor_body_idx = self._robot.body_names.index(self.cfg.ctrl.force_sensor_body_name)
        print(f"[ForgeEnv] force_sensor_body_name='{self.cfg.ctrl.force_sensor_body_name}'  idx={self.force_sensor_body_idx}")
    ...
    def _apply_action(self):
        ...
        # Yaw: symmetric mapping [-1, 1] -> [-180°, +180°] (full 360°, no dead zone), in fixed-asset local frame.
    ...
    def _reset_idx(self, env_ids):
        ...
        # Relative orientation to the bolt frame.
        ...
        # Inverse of the [-1,1] -> [-180°, +180°] yaw mapping in _apply_action: recover
        # the policy's yaw action from the current fingertip orientation in the fixed-asset
        # local frame. Required so EMA on the action buffer starts from the actual pose.
        ...
        fingertip_yaw_local = torch.where(fingertip_yaw_local > torch.pi, fingertip_yaw_local - 2 * torch.pi, fingertip_yaw_local)
        fingertip_yaw_local = torch.where(fingertip_yaw_local < -torch.pi, fingertip_yaw_local + 2 * torch.pi, fingertip_yaw_local)
        yaw_action = (fingertip_yaw_local + np.deg2rad(180.0)) / np.deg2rad(360.0) * 2.0 - 1.0

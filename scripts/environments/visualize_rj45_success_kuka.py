"""Visualize and interactively test the ForgeRJ45Insert success check — Kuka variant.
...
Keyboard controls (click the viewport once to give it focus):
  ...
  G                  — teleport virtual target to SUCCESS center position
                       (connector tip exactly at socket opening)

Coloured sphere markers (updated every frame):
  BLUE   — RJ45 connector tip (plug-local, tracked as held_base)
  ORANGE — RJ45 socket opening centre (fixed asset top face)
  GREEN  — 5 keypoints on plug connector face (held frame)
  YELLOW — 5 keypoints on socket opening (fixed frame)

Usage
-----
./isaaclab.sh -p scripts/environments/visualize_rj45_success_kuka.py --num_envs 1 --freeze_socket
...
parser = argparse.ArgumentParser(description="Interactive ForgeRJ45Insert Kuka success visualizer.")
...
# ---------------------------------------------------------------------------
# RJ45 geometry reference points (metres, post-STL→USD 0.001 scale).
# ---------------------------------------------------------------------------
# Connector TIP keypoints in link_rj45 local frame (sim_Z = base_height = -0.01398 m).
# These 5 points cover the male plug face centre and four corners.
_RJ45_TIP_KPS = [
    (-0.0105,  0.0097, -0.01398),  # face centre
    (-0.0292,  0.0188, -0.01398),  # left-front corner
    ( 0.0083,  0.0188, -0.01398),  # right-front corner
    (-0.0292,  0.0006, -0.01398),  # left-back corner
    ( 0.0083,  0.0006, -0.01398),  # right-back corner
]

# Socket opening keypoints in fixed-asset (female) local frame.
# Centred on the opening (fixed-local Z = 0.02922 m = height).
_SOCKET_KPS = [
    (-0.0105,  0.0097, 0.02922),  # centre
    (-0.0292,  0.0188, 0.02922),  # left-front
    ( 0.0083,  0.0188, 0.02922),  # right-front
    (-0.0292,  0.0006, 0.02922),  # left-back
    ( 0.0083,  0.0006, 0.02922),  # right-back
]

# Geometry constants (metres).
_RJ45_BASE_HEIGHT = -0.01398   # RJ45MaleCfg.base_height  (tip BELOW link_rj45 origin)
_GRASP_Z_OFF      =  0.050     # joint_rj45 z-offset in link_tcp frame
                                # → link_rj45 is 50 mm "below" link_tcp (in world Z)
...
# F → ENGAGED: plug tip 30 mm ABOVE socket opening (aligned for insertion).
# G → SUCCESS: connector tip exactly at socket opening Z.
...
def _teleport_vtgt_to(inner, label: str, z_above_success: float = 0.0) -> None:
    ...
    automatically tracks the socket even when position randomisation is enabled.

    Chain:  target_held_base_pos  (socket opening in world/env frame)
            → link_rj45  = held_base − [0,0, _RJ45_BASE_HEIGHT]
                         = held_base + [0,0, 0.01398]   (tip is below origin)
            → link_tcp   = link_rj45 + [0,0, _GRASP_Z_OFF]   (TCP is above rj45)
            → EE target  = link_tcp  + [0,0, z_above_success]
    """
    ...
    ee_pos[:, 2] += -_RJ45_BASE_HEIGHT + _GRASP_Z_OFF + z_above_success
...
def _get_markers() -> VisualizationMarkers:
    ...
    if _markers is None:
        cfg = VisualizationMarkersCfg(
            prim_path="/Visuals/RJ45Markers",
            markers={
                "tip_plug": sim_utils.SphereCfg(       # connector tip (plug)
                    radius=0.004,
                    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.2, 0.4, 1.0)),
                ),
                "tip_socket": sim_utils.SphereCfg(     # socket opening centre
                    radius=0.004,
                    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(1.0, 0.5, 0.0)),
                ),
                "kp_held": sim_utils.SphereCfg(        # plug face keypoints
                    radius=0.002,
                    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.2, 0.9, 0.2)),
                ),
                "kp_target": sim_utils.SphereCfg(      # socket opening keypoints
                    radius=0.002,
...
def _draw_rj45_markers(inner):
    ...
    # Slot 0: plug tip (blue), Slot 1: socket opening (orange).
    tip_loc     = torch.tensor(_RJ45_TIP_KPS[0], device=device).expand(num_envs, -1).clone()
    ...
    # Per-episode body keypoints from env (same logic as training).
    # GREEN = kp_rj45_local in plug frame, YELLOW = same offset in socket frame.
    # Overlap when plug origin == socket origin (fully inserted).
    if hasattr(inner, "kp_rj45_local"):
        for i in range(inner.kp_rj45_local.shape[1]):
...
def _print_success_info(inner, step, successes: torch.Tensor, engaged: torch.Tensor):
    ...
    held_pos   = inner.held_pos    # (N,3) link_rj45 origin, env-local
    held_quat  = inner.held_quat
    fixed_pos  = inner._fixed_asset.data.root_pos_w - inner.scene.env_origins
    fixed_quat = inner._fixed_asset.data.root_quat_w
    ...
    ident_q  = torch.tensor([1.0, 0.0, 0.0, 0.0], device=device).unsqueeze(0).expand(num_envs, -1)
    tip_loc  = torch.tensor([_RJ45_TIP_KPS[0][0], _RJ45_TIP_KPS[0][1], _RJ45_BASE_HEIGHT],
                             device=device).expand(num_envs, -1).clone()
    _, tip_env = torch_utils.tf_combine(held_quat, held_pos, ident_q, tip_loc)

    # Socket opening in env-local frame.
    sock_loc = torch.tensor(_SOCKET_KPS[0], device=device).expand(num_envs, -1).clone()
    _, sock_env = torch_utils.tf_combine(fixed_quat, fixed_pos, ident_q, sock_loc)

    # Gap: tip relative to socket opening, expressed in socket (fixed) frame.
    tip_in_socket = quat_apply_inverse(fixed_quat, tip_env - fixed_pos)
    sock_opening  = torch.tensor(_SOCKET_KPS[0], device=device)
    ...
    _, _, plug_yaw = _euler_xyz(held_quat[0:1])
    _, _, sock_yaw = _euler_xyz(fixed_quat[0:1])
    yaw_diff_deg = _math.degrees(
        float(((plug_yaw - sock_yaw + _math.pi) % (2 * _math.pi) - _math.pi).item())
    )

    # EE (link_tcp) orientation.
    tcp_quat = inner.fingertip_midpoint_quat[0:1]
    r, p, y  = _euler_xyz(tcp_quat)
    r_deg, p_deg, y_deg = _math.degrees(r.item()), _math.degrees(p.item()), _math.degrees(y.item())
    ...
    print(
        ...
        f"  link_rj45 pos (env-local): {_fmt(held_pos[0].cpu())}\n"
        ...
        f"  (<0 = inside socket)\n"
        f"  plug yaw vs socket yaw: {yaw_diff_deg:+.1f}°  (ENGAGE needs |yaw|<20°)\n"
        f"  (SUCCESS fires when |XY|<3 mm AND Z_gap < {inner.cfg_task.fixed_asset_cfg.height * inner.cfg_task.success_threshold * 1000:.1f} mm)"
    )
...
def main():
    ...
    env   = gym.make("Isaac-Forge-RJ45Insert-Kuka-Direct-v0", cfg=env_cfg)
    inner = env.unwrapped
    ...
    print(
        ...
        "[TELEPORT]  F=ENGAGED position (30 mm above socket)  G=SUCCESS center position\n"
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            if _diag_steps > 0:
                ...
                # held_base = link_rj45 + [0,0, _RJ45_BASE_HEIGHT]
                held_base_z = inner.held_pos[0, 2].item() + _RJ45_BASE_HEIGHT
                tgt_z       = tgt[0, 2].item()
                ee_pos      = inner.fingertip_midpoint_pos[0].cpu()
                vtgt        = _vtgt_pos[0].cpu() if _vtgt_pos is not None else ee_pos
                print(
                    f"  [diag {_diag_steps:3d}]"
                    f"  tip Z={held_base_z*1000:+7.2f} mm  socket Z={tgt_z*1000:+7.2f} mm"
                    f"  gap={(held_base_z - tgt_z)*1000:+6.2f} mm"
                    f"  | EE Z={ee_pos[2]*1000:+7.2f}  vtgt Z={vtgt[2]*1000:+7.2f}"
                )
                _diag_steps -= 1

            # engage_threshold=2.0 → alignment check; success_threshold=0.1 → inside socket
